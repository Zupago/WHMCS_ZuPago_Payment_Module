...
$apikey = $GATEWAY["zupago_pass"];
if ($_REQUEST['page'] == 'cancel') {
    ...
    if ($apikey == $_POST['ZUPAYEE_ACC_KEY']) { // proccessing payment if only apikey is valid
        ...
        if (!$qry) {
            logTransaction($GATEWAY["name"], array_merge($_POST, array('SQL query' => $qry)), "SQL query error");
            //   die();
            $paymentSuccess = false;
        }
        if (mysql_num_rows($qry) != 1) {
            logTransaction($GATEWAY["name"], $_POST, "SQL returned invalid data");
            // die();
            $paymentSuccess = false;
        }
        ...
        if (!empty($GATEWAY['convertto']))
            if ($data['currency_id'] != $GATEWAY['convertto']) { // need to convert to another currency
